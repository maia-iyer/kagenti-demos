#!/usr/bin/env bash
# Delete the claude-crash-demo pod, so the Deployment recreates a fresh one.
# This is the "platform kill" — analogous to kill -9 in the local demo, but the
# signal comes from the scheduler, not from a signal to a PID.
#
# Usage: ./kill-by-pod.sh [namespace] [--force]
#   namespace   defaults to "default"
#   --force     passes --grace-period=0 --force to kubectl (skip graceful shutdown)
#
