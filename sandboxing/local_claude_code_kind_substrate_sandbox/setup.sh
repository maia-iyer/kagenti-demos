...
echo ""
echo "     The PreToolUse hook denies any local Bash call that isn't routed"
echo "     through the sandbox binary, so Claude will retry through it."
echo "     Smoke test: 'uname -a' should print Linux (Alpine), not Darwin."
echo "     See README 'Verify the sandbox from inside Claude' for a workspace"
echo "     round-trip test using a small run.sh + note.txt."
