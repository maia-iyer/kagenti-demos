...
pod="$(kubectl get pod -n "$namespace" -l app=claude-crash-demo,role=claimed -o jsonpath='{.items[0].metadata.name}' 2>/dev/null || true)"

if [ -z "$pod" ]; then
  echo "no pod with label app=claude-crash-demo,role=claimed in namespace=$namespace" >&2
  exit 1
fi
